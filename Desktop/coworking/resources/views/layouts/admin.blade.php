...
<head>
    ...
    <title>@yield('title', 'SpaceHive Admin') — Administration</title>
...
<aside class="sidebar" id="sidebar">
    <div style="padding: 20px 16px;">
        <!-- Logo -->
        <a href="{{ route('admin.dashboard') }}" style="display: flex; align-items: center; gap: 10px; margin-bottom: 24px; text-decoration: none;">
        ...
        <div style="background: var(--surface3); border-radius: 10px; padding: 10px 12px; margin-bottom: 20px; border: 1px solid var(--border);">
            <div style="display: flex; align-items: center; gap: 10px;">
                <div style="width: 34px; height: 34px; border-radius: 8px; background: linear-gradient(135deg, #4cbcbe, #ff8247); display: flex; align-items: center; justify-content: center; font-weight: 700; color: white; font-size: 13px; flex-shrink: 0;">
                ...
                <div style="min-width: 0;">
                    ...
                    <div style="font-size: 11px; color: var(--text-muted);">Administrateur</div>
                </div>
            </div>
        </div>
        ...
        <nav>
            ...
            <a href="{{ route('admin.spaces.index') }}" class="sidebar-link {{ request()->routeIs('admin.spaces.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.promo') }}" class="sidebar-link {{ request()->routeIs('admin.promo*') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.avis') }}" class="sidebar-link {{ request()->routeIs('admin.avis*') ? 'active' : '' }}">
                ...
                Avis
                @php $avisCount = \App\Models\Avis::count(); @endphp
                @if($avisCount > 0)
                <span style="margin-left:auto;background:rgba(245,158,11,0.15);color:#d97706;font-size:10px;font-weight:700;padding:2px 7px;border-radius:10px;">{{ $avisCount }}</span>
                @endif
            </a>
        </nav>
...
<div class="main-content">
    ...
    <header style="background: var(--surface2); border-bottom: 1px solid var(--border); padding: 0 24px; height: 58px; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 40;">
        <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <h1 style="font-family: 'Poppins', sans-serif; font-size: 18px; font-weight: 700; color: var(--text);">@yield('page-title', 'Administration')</h1>
        </div>
        <div style="display: flex; align-items: center; gap: 10px;">
            <span style="font-size: 12px; background: rgba(79,126,247,0.1); color: var(--accent); padding: 4px 12px; border-radius: 20px; font-weight: 600;">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="display:inline;vertical-align:middle;margin-right:4px;"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                Admin
            </span>
        </div>
    </header>
